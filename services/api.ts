...
const mockSimilarTickets: SimilarTicket[] = [
    { 
        ticket_no: 'T001', 
        problem_description: 'Cannot login to VPN, password reset link not working.', 
        solution_text: 'The VPN uses a separate directory. Please reset your password at vpn.company.com/reset.', 
    ...
    { 
        ticket_no: 'T002', 
        problem_description: 'Login password for my computer is not being accepted after I changed it.', 
        solution_text: 'Ensure you are connected to the office network or VPN for the new password to sync. If issue persists, contact IT.', 
    ...
    { 
        ticket_no: 'T003', 
        problem_description: 'I am unable to access the sales dashboard, it says "access denied".', 
        solution_text: 'Access to the sales dashboard requires approval from your manager. Please submit an access request ticket.', 
        similarity_score: 0.85, 
        category: 'Software',
        created_time: new Date(Date.now() - 86400000 * 10).toISOString() 
    },
];
...
export const getTicketsByCategory = (category: string): Promise<SimilarTicket[]> => {
    return new Promise(resolve => {
        if (!isModelTrained) {
            resolve([]);
        } else {
    ...
    return new Promise(async (resolve, reject) => {
        uploadedTickets = rawParsedTickets.map((rawTicket, index): SimilarTicket => {
            ...
            return {
                ...
                priority: finalPriorityMapping[rawTicket.priority] || calculatedPriority,
...
export const getCategoryPriorityBreakdownData = (): Promise<CategoryPriorityBreakdownData[]> => {
    return new Promise(resolve => {
        setTimeout(() => {
            if (isModelTrained && dynamicCategories.length > 0) {
                const priorityMap: Record<string, { p1: number, p2: number, p3: number }> = {};

                // Initialize map for all dynamic categories to ensure they appear on the chart
                dynamicCategories.forEach(cat => {
                    priorityMap[cat] = { p1: 0, p2: 0, p3: 0 };
                });

                uploadedTickets.forEach(ticket => {
                    const category = ticket.category;
                    const priority = ticket.priority as 'p1' | 'p2' | 'p3' | undefined;
                    if (category && priority && priorityMap[category]) {
                        if (priority in priorityMap[category]) {
                            priorityMap[category][priority]++;
                        }
                    }
                });

                const data = Object.entries(priorityMap)
                ...
                // Mock data
                resolve([
                    { name: 'Software', p1: 15, p2: 120, p3: 200 },
                    { name: 'Hardware', p1: 30, p2: 40, p3: 50 },
                    { name: 'Network', p1: 25, p2: 80, p3: 150 },
                    { name: 'Account Management', p1: 5, p2: 30, p3: 100 },
                    { name: 'Database', p1: 10, p2: 25, p3: 30 },
                ]);
            }
        }, 900);
    });
