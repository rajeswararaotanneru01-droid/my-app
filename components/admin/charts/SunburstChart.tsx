...
const ProblemClusterChart: React.FC<ProblemClusterChartProps> = ({ data, onNodeClick }) => {
  ...
  useEffect(() => {
    ...
    nodeGroup.append("text")
        .text((d: any) => SAP_MODULE_FULL_NAMES[d.name] || d.name)
        .attr('x', 0)
        .attr('y', 0)
        .attr('dy', '0.35em')
  ...
  const headerText = hoveredNode ? `${SAP_MODULE_FULL_NAMES[hoveredNode.name] || hoveredNode.name}: ${hoveredNode.value.toLocaleString()} tickets` : `All Issues: ${nodes[0]?.value.toLocaleString() || 0} tickets`;
